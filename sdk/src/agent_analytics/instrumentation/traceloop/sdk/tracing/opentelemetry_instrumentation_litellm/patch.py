...
from .langtrace_utils_for_litellm import (
    LLMSpanAttributes,
    SpanAttributes,
    silently_fail,
    SERVICE_PROVIDERS,
...
def should_suppress_litellm_instrumentation(kwargs: Dict[str, Any]) -> bool:
    """
    Determine if LiteLLM instrumentation should be suppressed for OpenAI/Azure calls.

    LiteLLM acts as a routing layer to various providers. When calling OpenAI or Azure OpenAI,
    if the OpenAI instrumentor is active, we suppress LiteLLM instrumentation to avoid
    duplicate spans with the same content.

    Args:
        kwargs: The keyword arguments passed to the LiteLLM function

    Returns:
        True if instrumentation should be suppressed, False otherwise.
    """

    # Check if OpenAI package is installed (used by both OpenAI and Azure)
    if not is_package_installed("openai"):
        return False

    # Strategy 1: Check model name for OpenAI/Azure prefix
    model = kwargs.get("model", "").lower()

    # Check if model starts with openai or azure prefix
    if model.startswith("openai") or model.startswith("azure"):
        return True

    # Strategy 2: Check base_url for OpenAI or Azure endpoints
    base_url = kwargs.get("base_url", "").lower()
    if base_url:
        # OpenAI endpoints
        if "api.openai.com" in base_url:
            return True
        # Azure OpenAI endpoints (cognitive.microsoft.com or openai.azure.com)
        if "cognitive.microsoft.com" in base_url or "openai.azure.com" in base_url:
            return True
...
def chat_completions_create(version: str, tracer: Tracer) -> Callable:
    ...
    def traced_method(
        ...
        # Suppress LiteLLM instrumentation if OpenAI/Azure and OpenAI instrumentor is active
        if should_suppress_litellm_instrumentation(kwargs):
