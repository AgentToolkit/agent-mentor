...
class TraceloopCallbackHandler(BaseCallbackHandler):
        ...
        **kwargs: Any,
        ...
        if metadata.get("langgraph_node") == name:
            ...
            if name in graph.builder.nodes:
                ...
                action = _get_action_from_func(node_func, kind=ActionKind.TOOL)

                # Override name if different from function name
                if action.name != name:
                    action.name = name

                # Report to current span (no new span created)
                action._report_to_span(span=span)
    ...
    def on_tool_start(
        ...
        **kwargs: Any,
        ...
        if tool_func:
            action = _get_action_from_func(tool_func, kind=ActionKind.TOOL)
            # Override name if different from function name
            if action.name != name:
                action.name = name
