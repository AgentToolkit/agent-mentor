...
class TestNewTraceOnWorkflow(unittest.TestCase):
    ...
    def test_with_new_trace_on_workflow(self):
        ...
        # With new_trace_on_workflow=False, each invocation should not have its own trace ID
        trace_ids = {span.context.trace_id for span in spans}
        self.assertEqual(
            len(trace_ids), 1,
            "With new_trace_on_workflow=False, each invoke of agent span should have same trace ID"
        )
