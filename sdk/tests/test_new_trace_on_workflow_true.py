...
class TestNewTraceOnWorkflow(unittest.TestCase):
    ...
    def test_with_new_trace_on_workflow(self):
        """Test that new_trace_on_workflow=True creates separate traces for each workflow invocation."""
        # Run workflow
        ...
        self.assertEqual(
            len(invoke_spans), 2,
            "Expected at least 2 spans for LangGraph.workflow"
        )

        # With new_trace_on_workflow=True, each invocation should have its own trace ID
        trace_ids = {span.context.trace_id for span in spans}
        self.assertEqual(
            len(trace_ids), len(invoke_spans)+1,  # Add 1 for the outer "test" span
            "With new_trace_on_workflow=True, each invoke span should have a unique trace ID"
        )
